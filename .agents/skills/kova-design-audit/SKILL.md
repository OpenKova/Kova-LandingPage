---
name: kova-design-audit
description: Audit Kova Agent frontend code and rendered interfaces for Carapace drift, token misuse, primitive reimplementation, accessibility problems, responsive defects, and off-brand copy. Use for design reviews, compliance checks, or scheduled audit-and-fix workflows.
---

# Kova Agent Design Audit
...
3. Read the version-matched token contract and consumer adapters from the
   installed product guidance skill: `kova-carapace` for new installs or
   the `kova-design-system` compatibility alias for an upgraded lock.
