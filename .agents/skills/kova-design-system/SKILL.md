---
name: kova-design-system
description: Compatibility alias for existing Kova Agent installations that now applies Carapace semantic tokens, themes, shared CSS foundations, consumer adapters, and established local primitives.
---
...
`@kova/carapace` when it is installed, otherwise preserve the legacy
`@kova/design-system` specifier until the dependency is migrated.
