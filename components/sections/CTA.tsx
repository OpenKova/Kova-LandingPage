...
const cards: { name: string; sub: string; href: string; icon: ReactNode }[] = [
  {
    name: "Discord",
    sub: "Join the community",
    href: "https://discord.gg/eBHFpuYCMQ",
    icon: (
...
export function CTA() {
  return (
    <section id="cta" className="cta-section">
      <nav className="cta-grid" aria-label="Community and resources">
        {cards.map((c) => (
          <a key={c.name} href={c.href} target="_blank" rel="noopener" className="cta">
            {c.icon}
            <span className="cta-label">{c.name}</span>
            <span className="cta-sub">{c.sub}</span>
          </a>
        ))}
      </nav>
    </section>
